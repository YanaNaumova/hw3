...
function Rating() {
  ...
  return (
    <div className={container}>
      <img src={ratingList[ratingValue]} alt="reiting" />
      <div className={btnContainer}>
        <button onClick={() => updateRatingValue(0)}>Плохо</button>
        <button onClick={() => updateRatingValue(1)}>Приемлимо</button>
        <button onClick={() => updateRatingValue(2)}>Хорошо</button>
        <button onClick={() => updateRatingValue(3)}>Отлично</button>
      </div>
    </div>
  );
}
